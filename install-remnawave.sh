#!/bin/bash
# =============================================================================
# RemnaWave Node Auto-Installer v2.4 — Bash-compatible syntax
# =============================================================================
...
# =============================================================================
# 🔧 FIX: Обработка broken apt репозиториев (упрощённая совместимая версия)
# =============================================================================
fix_broken_repos() {
    log_info "Проверяю apt репозитории..."
    
    # Пробуем update, ловим вывод
    local update_output
    ...
    if echo "$update_output" | grep -qiE "release|404|not found"; then
        ...
        # Отключаем известные проблемные репозитории
        ...
        # Пробуем update ещё раз с флагом
    ...
    if ! command -v docker &> /dev/null; then
        ...
        if curl -fsSL https://get.docker.com -o /tmp/install-docker.sh 2>/dev/null; then
            if sh /tmp/install-docker.sh 2>&1; then
                log_success "Docker установлен через официальный скрипт"
            else
                log_warn "Официальный скрипт завершился с ошибкой, пробую apt..."
                apt-get install -y docker.io docker-compose 2>/dev/null || true
            fi
            rm -f /tmp/install-docker.sh
        else
            log_warn "Не удалось скачать скрипт, пробую apt..."
            apt-get install -y docker.io docker-compose 2>/dev/null || true
        fi
        ...
        if command -v docker &> /dev/null; then
            ...
            log_warn "Попробуйте вручную: curl -fsSL https://get.docker.com | sh"
            return 1
        fi
    else
...
EOF
    ...
    log_info "Запускаю..."; docker compose up -d 2>/dev/null || docker-compose up -d 2>/dev/null || true
    sleep 3
    docker ps --format '{{.Names}}' 2>/dev/null | grep -q beszel-agent && { log_success "Запущен"; mark_installed "beszel_agent"; } || log_warn "Проверьте логи"
}
    ...
    while true; do
        ...
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            ...
            if [[ "$CURRENT_KEYS" -eq 0 ]]; then
                log_error "❌ Файл пуст! Не перезапускаю SSH!"
                continue
            fi
            ...
            log_info "Отменено. Для ручного рестарта: systemctl restart ssh"
            return 0
        fi
    done
...
main() {
    echo -e "${GREEN}╔════════════════════════════════╗${NC}"
    echo -e "${GREEN}║${NC}  RemnaWave Installer v2.4  ${GREEN}║${NC}"
    ...
    echo "  5. Beszel  6. 🔐 SSH (Safe)  7. iptables"; echo ""
